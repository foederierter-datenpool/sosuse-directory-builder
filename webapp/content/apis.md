...
Endpoint: `https://sosue.correlaid.org/directory/sparql`

<a href="https://sosue.correlaid.org/directory/sparql?query=ASK%7B%7D" target="_blank" rel="noopener noreferrer">Try an ASK query</a>, or count Einrichtungen:
...
curl --get \
  ...
  https://sosue.correlaid.org/directory/sparql
```

Using the `SERVICE` keyword, you can query this endpoint from any SPARQL editor connected to a server that supports federated queries. <a href="https://www.govdata.de/sparql-assistent#query=SELECT%20(COUNT(DISTINCT%20%3Fplace)%20AS%20%3Fcount)%20WHERE%20%7B%0A%20%20SERVICE%20%3Chttps%3A%2F%2Fsosue.correlaid.org%2Fdirectory%2Fsparql%3E%20%7B%0A%20%20%20%20%3Fplace%20a%20%3Chttp%3A%2F%2Fwww.w3.org%2Fns%2Forg%23OrganizationalUnit%3E%20.%0A%20%20%7D%0A%7D&amp;endpoint=https%3A%2F%2Fwww.govdata.de%2Fsparql&amp;requestMethod=GET&amp;tabTitle=Sosuse&amp;headers=%7B%7D&amp;contentTypeConstruct=application%2Fn-triples%2C*%2F*%3Bq%3D0.9&amp;contentTypeSelect=application%2Fsparql-results%2Bjson%2C*%2F*%3Bq%3D0.9&amp;outputFormat=table" target="_blank" rel="noopener noreferrer">Try it in GovData’s SPARQL assistant</a>.
...
[Open Swagger to explore the methods and try them out](https://sosue.correlaid.org/swagger-ui.html), or view the [OpenAPI description](https://sosue.correlaid.org/v3/api-docs).

## GraphQL: select fields and follow relationships

The read-only GraphQL API lets you choose fields and follow connections, such as Einrichtungen and their addresses, in one request. [Open GraphiQL for schema documentation and a query editor](https://sosue.correlaid.org/graphiql?query=%7B%0A%20%20einrichtungSchema%28limit%3A%205%29%20%7B%0A%20%20%20%20id%0A%20%20%20%20name%0A%20%20%20%20address%20%7B%20postalCode%20addressLocality%20%7D%0A%20%20%7D%0A%7D). Queries go to `/graphql` on the same host.
